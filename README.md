...
3. **`quality_check.py`** — an automated pre-upload gate, since nobody is watching this run in real time. It verifies the render is actually sound (has both video and audio streams, correct resolution, audio isn't silent, frames aren't blank/frozen, thumbnails are the right size, title/description/tags are within YouTube's length limits) and does a copyright/licensing check (the TTS voice used is on the approved list, and no media file shows up in the video's output folder that isn't accounted for in `asset_manifest.json`). If this fails, that video's upload is skipped entirely — see "Quality and copyright checks" below for the full reasoning and how to extend it once you add real B-roll/music.
4. Upload to YouTube as **private**, scheduled to auto-publish `PUBLISH_DELAY_HOURS` later (default 6). This is the remaining safety margin — it gives you a window to catch and pull anything that looks wrong before it goes public, without blocking the pipeline on you being available at upload time.
5. Everything (scripts, audio, video, thumbnails, SEO json, quality reports, a `cycle_summary.json`) gets attached to the GitHub Actions run as a downloadable artifact, whether or not the upload succeeded — so nothing is ever silently lost, and you always have a paper trail to review even if you set the publish delay to something short.
...
Why this exists: in a fully unattended pipeline there's no human watching each video before it uploads, and the publish delay only protects you if someone actually checks during that window. `quality_check.py` catches the specific things a human would catch instantly on a quick watch — dead/silent audio (a broken TTS call that produced an empty file), a frozen or blank render (a rendering bug that produced a static frame instead of the real video), a corrupt or wrong-sized thumbnail, or metadata that would get truncated or rejected by YouTube. It runs after SEO selection and before upload; a failure skips that video's upload entirely (the other videos in the run still proceed) and gets logged in `<stem>_quality_report.json` plus the run's `cycle_summary.json`.

The copyright side of it is narrower right now than "checks for copyrighted content" — it can't watch the video and tell whether something is infringing. What it actually does: verifies the TTS voice used is one of the approved Edge-TTS neural voices (not some other, differently-licensed voice source), and fails closed if it finds any media file in a video's output folder that isn't either a known pipeline-generated file or explicitly logged in `pipeline/asset_manifest.json` with a license. Right now that manifest's B-roll/music list is empty by design, because the pipeline doesn't use any external footage or music yet — everything on screen is generated from code (gradients, text, charts). If you add real stock footage or music later (see "Current visual style, and how to upgrade it" above), you have to log each asset's source and license in `asset_manifest.json` first, or `quality_check.py` will fail every video that uses it. That's intentional — it turns "did I remember to check the license on this clip" into something the pipeline enforces rather than something you have to remember every single day.

Both halves were tested in-session against a real rendered test video: all checks passed on a clean video, and — to confirm the checks actually catch problems rather than just always passing — an unapproved voice name and an unlogged extra media file were both correctly flagged as failures.
